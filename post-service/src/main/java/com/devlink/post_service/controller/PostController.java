...
public class PostController {
        ...
        @GetMapping("/images")
        public ResponseEntity<Page<String>> getImages(
                        @RequestParam(required = false) Long userId,
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "10") @Max(20) int size) {
                return ResponseEntity.ok(postService.getImages(userId, page, size));
        }

        @GetMapping("/images/details")
        public ResponseEntity<Page<MediaResponse>> getImagesDetails(
                        @RequestParam(required = false) Long userId,
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "10") @Max(20) int size) {
                return ResponseEntity.ok(postService.getImagesDetails(userId, page, size));
        }

        @GetMapping("/images/{imageId}")
        public ResponseEntity<ApiResponse<MediaResponse>> getImageDetail(
                        @PathVariable Long imageId,
                        @RequestParam(required = false) Long userId) {
                return ResponseEntity.ok(ApiResponse.ok(postService.getImageDetail(imageId, userId), SUCCESS));
        }
