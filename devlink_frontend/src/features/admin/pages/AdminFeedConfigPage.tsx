...
export default function AdminFeedConfigPage() {
    ...
    const renderCard = (key: string) => {
        ...
        if (!meta) return null;
        if (!config) return (
            <div key={key} style={{ background: '#F9FAFB', borderRadius: 18, padding: 22, border: '1px dashed #E5E7EB', opacity: 0.6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: meta.bg, display: 'grid', placeItems: 'center', color: meta.color }}>{meta.icon}</div>
                    <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#9CA3AF' }}>{meta.label}</div>
                        <div style={{ fontSize: 12, color: '#D1D5DB' }}>{key} — not yet in DB</div>
                    </div>
                </div>
            </div>
        );
        ...
        return (
            <div key={key} style={{ background: '#fff', borderRadius: 18, padding: 22, boxShadow: '0 4px 20px rgba(15,23,42,0.06)', border: `1px solid ${isDirty ? '#BFDBFE' : '#F1F5F9'}`, transition: 'border-color 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: meta.bg, display: 'grid', placeItems: 'center', color: meta.color, flexShrink: 0 }}>{meta.icon}</div>
                    <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#111827' }}>{meta.label}</div>
                        <div style={{ fontSize: 12, color: '#9CA3AF' }}>{key}</div>
                    </div>
                    {isDirty && <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20, background: '#DBEAFE', color: '#1D4ED8' }}>unsaved</span>}
                </div>
                <div style={{ fontSize: 12, color: '#6B7280', marginBottom: 14, padding: '8px 12px', background: '#F9FAFB', borderRadius: 10, borderLeft: `3px solid ${meta.color}` }}>{meta.hint}</div>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <input id={`config-${key}`} type="number" step={key === 'interest.decay_rate' ? '0.01' : '0.5'} min={0.01} max={key === 'interest.decay_rate' ? 0.99 : 100}
                        ...
                        style={{ flex: 1, padding: '10px 14px', borderRadius: 12, border: `1px solid ${isDirty ? '#93C5FD' : '#E5E7EB'}`, fontSize: 15, fontWeight: 700, color: '#111827', outline: 'none', boxSizing: 'border-box', background: isDirty ? '#EFF6FF' : '#fff' }}
                    />
                    <button type="button" id={`save-${key}`} onClick={() => { void handleSave(key); }} disabled={saving[key] || !isDirty}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: 12, border: 'none', background: isDirty ? meta.color : '#F3F4F6', color: isDirty ? '#fff' : '#9CA3AF', cursor: isDirty ? 'pointer' : 'default', fontWeight: 700, fontSize: 13 }}>
                        <Save size={14} />{saving[key] ? 'Saving...' : 'Save'}
                    </button>
                </div>
                {config.updatedAt && <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 10 }}>Last updated: {formatDate(config.updatedAt)}{config.updatedBy && ` · by admin #${config.updatedBy}`}</div>}
            </div>
        );
    };
    ...
    return (
        <div style={{ fontFamily: "'Inter', sans-serif" }}>
            <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#111827' }}>Feed Scoring Config</h1>
                    <p style={{ margin: '4px 0 0', fontSize: 13, color: '#6B7280' }}>Configure interest scoring weights and feed generation parameters. Changes take effect within 10 minutes.</p>
                </div>
                <button type="button" id="refresh-feed-config" onClick={() => { void loadConfigs(); }} disabled={loading}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderRadius: 10, flexShrink: 0, background: '#F3F4F6', color: '#111827', border: '1px solid #E5E7EB', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>
                    <RefreshCw size={15} style={{ animation: loading ? 'spin 1s linear infinite' : undefined }} /> Refresh
                </button>
            </div>

            {status && <div style={{ marginBottom: 20, padding: '13px 18px', borderRadius: 12, background: status.type === 'success' ? '#ECFDF5' : '#FEF2F2', color: status.type === 'success' ? '#166534' : '#B91C1C', border: `1px solid ${status.type === 'success' ? '#BBF7D0' : '#FECACA'}`, fontWeight: 600, fontSize: 13 }}>{status.message}</div>}

            {error && <div style={{ marginBottom: 20, padding: '13px 18px', borderRadius: 12, background: '#FEF2F2', color: '#B91C1C', border: '1px solid #FECACA', fontWeight: 600, fontSize: 13 }}>{error}</div>}
                    ...
                    <section style={{ marginBottom: 28 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: 12, color: '#9CA3AF' }}>Points added per interaction type.</div>
                            </div>
                        </div>
                    ...
                    <section>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: 12, color: '#9CA3AF' }}>Control how the personalized feed is built.</div>
                            </div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
