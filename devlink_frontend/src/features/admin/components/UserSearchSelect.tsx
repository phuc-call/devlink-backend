...
export default function UserSearchSelect({ value, onChange, placeholder = 'Tìm theo username hoặc email...' }: Props) {
        ...
        <div ref={containerRef} style={{ position: 'relative' }}>
            {value ? (
                <div style={{
                    ...
                    {value.avatarUrl ? (
                        <img src={value.avatarUrl} alt="" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover' }} />
                    ) : (
                        <div style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{value.username}</div>
                        <div style={{ fontSize: 12, color: '#6B7280' }}>{value.email}</div>
                    </div>
                    <span style={{
                        padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 700,
                        background: BADGE_COLORS[value.badge].bg,
                        color: BADGE_COLORS[value.badge].color,
                        border: `1px solid ${BADGE_COLORS[value.badge].border}`,
                    }}>
                        {BADGE_LABELS[value.badge]}
                    </span>
                    <button type="button" onClick={handleClear} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 4, color: '#6B7280' }}>
                        <X size={16} />
                    </button>
                </div>
            ) : (
            ...
            {open && results.length === 0 && !searching && keyword.trim() && (
                <div style={{
                    position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100,
                    background: '#fff', borderRadius: 12, marginTop: 4,
